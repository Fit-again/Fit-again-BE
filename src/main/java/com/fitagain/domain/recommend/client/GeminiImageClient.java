...
public class GeminiImageClient {
     ...
     * 정면 사진 + 디테일 사진 전부를 참고 이미지로 넣고, 리폼 작업 내용을 편집 지시로 전달합니다.
     */
    public byte[] generateReformAfterImage(String frontImageUrl, List<String> detailImageUrls, List<RecommendedWorkDto> recommendedWorks) {
        ...
        String prompt = """
                ...
                가방의 기본 형태, 소재, 색상은 유지하면서 아래 내용만 적당한 수준으로 반영하고, 과도한 디자인 변경은 하지 마세요.

                리폼 작업 내용:
                %s
                """.formatted(worksSummary);
     ...
     */
    public byte[] generateUpcyclingImage(String frontImageUrl, List<String> detailImageUrls, UpcyclingCandidateDto candidate) {
        String prompt = """
                ...
                """.formatted(candidate.getItemName(), candidate.getDescription());
